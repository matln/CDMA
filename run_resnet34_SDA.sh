...
target_egs_dir=exp/egs/${feature}-voices_dev_label-400-sequential-novad

stage=0
endstage=0

# --------------------------------- training (pytorch) ------------------------------------ #
model_dir="SDA_resnet34_${feature}"
...
# Extract embeddings on another computer with 8 GPUs (for running in parallel with the training stage)
[[ "$?" != "0" ]] && exit 1
...
if [[ $stage -le 1 && 1 -le $endstage ]]; then
  "${SUBTOOLS}"/runPytorchLauncher.sh --endstage 1 --master_addr "127.0.0.1" --pdb "false" \
    run_resnet34.py \
      ...
      --extract-data="voices_dev_label_cmn" \
...
if [[ $stage -le 2 && 2 -le $endstage ]]; then
  ...
  local/gather_results.sh --prefix $feature --score plda --lda true --submean true --vectordir $vectordir --evalset "voices_dev_label_cmn" \
    --metric "minDCF" --task "voices_dev" --task_name "voices_dev1" --epochs "$epochs" --positions "near" \
    --trainset "voices_dev_label_cmn" --submean_trainset "voices_dev_label_cmn" --force true
...
if [[ $stage -le 4 && 4 -le $endstage ]]; then
  ...
  local/gather_results.sh --prefix $feature --score plda --lda true --submean true --vectordir $vectordir \
    --evalset "voices_eval_cmn" --metric "minDCF" --task "voices_eval" --task_name "voices_eval1" --epochs "$epochs" \
    --positions "near" --trainset "voices_dev_label_cmn" --submean_trainset "voices_dev_label_cmn" --force true
