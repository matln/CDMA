...
def check_gpus():
    ...
    if not torch.cuda.is_available():
        ...
        with os.popen('nvidia-smi -h') as p :
            if not 'NVIDIA System Management' in p.read():
...
if check_gpus():
    ...
    def query_gpu(qargs=[]):
        ...
        p = os.popen(cmd)
        results = p.readlines()
        p.close() # Should be closed to avoid defunct subprocess [Snowdar 2019.07.19]
    ...
    def by_power(d):
        ...
        if any(v==1 for v in power_infos):
            logger.info('Power management unable for GPU {}'.format(d['index']))
            return 1
    ...
    class GPUManager():
        ...
        def _sort_by_memory(self,gpus,by_size=False):
            if by_size:
                #logger.info('Sorted by free memory size')
                return sorted(gpus,key=lambda d:d['memory.free'],reverse=True)
            else:
                #logger.info('Sorted by free memory rate')
        ...
        def auto_choice(self,mode=0):
            ...
            if mode==0:
                logger.info('Select GPU by largest free memory...')
                chosen_gpu=self._sort_by_memory(unspecified_gpus,True)[0]
            elif mode==1:
                logger.info('Select GPU by highest free memory rate...')
                chosen_gpu=self._sort_by_memory(unspecified_gpus,False)[0]
            elif mode==2:
                logger.info('Select GPU by by power...')
                chosen_gpu=self._sort_by_power(unspecified_gpus)[0]
            else:
                logger.info('mode is invalid, so default it by memory')
                chosen_gpu=self._sort_by_memory(unspecified_gpus)[0]
            chosen_gpu['specified']=True
            index=chosen_gpu['index']
            logger.info('Using GPU {i}:\n#### {info}'.format(i=index,info='\n#### '.join([str(k)+':'+str(v) for k,v in chosen_gpu.items()])))
            return int(index)
else:
