...
class Trainer(SimpleTrainer):
    ...
    def train_one_batch(self, s_batch, t_batch):
        ...
        loss_mmd = model.get_mmd_loss(s_feats, t_feats)
        loss_mmd = 1 * loss_mmd
        ...
        return loss_cls.detach().item(), loss_mmd.detach().item(), accuracy
        ...
        try:
            ...
            for this_epoch in range(start_epoch, epochs):
                for this_iter, (s_batch, t_batch) in enumerate(zip(data.s_train_loader, data.t_train_loader), 0):
                    ...
                    loss_cls, loss_mmd, acc = self.train_one_batch(s_batch, t_batch)
                    ...
                    if utils.is_main_training():
                        if valid_computed or (data.valid_loader and self.reporter.is_report(self.training_point)):
                            ...
                            real_snapshot = {"loss_cls": loss_cls, "loss_mmd": loss_mmd, "valid_loss": valid_loss,
                                             "train_acc": acc * 100, "valid_acc": valid_acc * 100}
                            snapshot = {"loss_cls": "{0:.6f}".format(loss_cls),
                                        "loss_mmd": "{0:.6f}".format(loss_mmd),
                            ...
                            real_snapshot = {"loss_cls": loss_cls, "loss_mmd": loss_mmd, "train_acc": acc * 100}
                            snapshot = {"loss_cls": "{0:.6f}".format(loss_cls),
                                        "loss_mmd": "{0:.6f}".format(loss_mmd), "valid_loss": "",
