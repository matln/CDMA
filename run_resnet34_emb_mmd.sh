...
model_dir="emb_mmd_resnet34_${feature}"
...
[[ "$?" != "0" ]] && exit 1
# extract_data="voices_dev_cmn,voxceleb1_train_cmn"
extract_data="voices_dev_label_cmn"
epochs="$(seq -s "," 30)"
time=
: =${time:=$(cat timestamp)}
...
if [[ $stage -le 2 && 2 -le $endstage ]]; then
  ...
  # submean_trainset="voices_dev_cmn"
  ...
  local/gather_results.sh --prefix $feature --score plda --lda true --submean true --vectordir $vectordir --evalset "voices_dev_cmn" \
    ...
    --trainset "$trainset" --submean_trainset "$submean_trainset" --force true
...
if [[ $stage -le 4 && 4 -le $endstage ]]; then
  ...
  local/gather_results.sh --prefix $feature --score plda --lda true --submean true --vectordir $vectordir \
    ...
    --positions "near" --trainset "$trainset" --submean_trainset "$submean_trainset" --force true
