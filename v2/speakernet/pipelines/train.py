...
if args.resume_training:
    ...
    if utils.is_main_training() and not args.debug:
        logger.info(
            "Please provide a brief description for the experiment, or Enter nothing:"
        )
        description = utils.input_with_timeout(timeout=300)

    # It is used for model.step() defined in model blueprint.
    if (
        hparams["lr_scheduler_params"]["name"] == "warmR"
        and hparams["encoder_params"]["use_step"]
    ):
        ...
        overrides.append(
            "encoder_params.step_params.T={}".format(
                hparams["encoder_params"]["step_params"]["T"]
            )
        )
...
# >>> Set seed
utils.set_seed(hparams["seed"], deterministic=True if "deterministic" not in hparams else hparams["deterministic"])
