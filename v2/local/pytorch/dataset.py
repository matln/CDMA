...
class SpeakerAwareWaveDataset(WaveDataset):
    ...
    def __getitem__(self, index):
        ...
        if label_multiplier == 1:
            sig_clean, multiplier = self.speed_down(sig)
        elif label_multiplier == 2:
            sig_clean, multiplier = self.speed_up(sig)
        else:
            sig_clean = sig

        if self.aug is not None:
            sig, multiplier = self.aug(sig_clean)
            assert multiplier == 0
        ...
        if self.chunk_position:
            if sig_clean.size(1) > self.chunk_size:
                start = random.randint(0, sig_clean.size(1) - self.chunk_size)
                sig_clean = sig_clean[:, start : start + self.chunk_size]
            else:
                pad_warp_num = self.chunk_size // sig_clean.size(1)
                pad_size = self.chunk_size % sig_clean.size(1)
                cat_list = [sig_clean for _ in range(pad_warp_num)]
                if pad_size != 0:
                    pad_start = random.randint(0, sig_clean.size(1) - pad_size)
                    pad_chunk = sig_clean[:, pad_start : pad_start + pad_size]
                    cat_list.append(pad_chunk)
                sig_clean = torch.cat(cat_list, dim=1)

        if not self.valid:
            # state will be omitted when num_workers=0, i.e., SingleProcessDataLoader
            state = self.get_random_state()
            return sig.squeeze(0), sig_clean.squeeze(0), label, f"{label_multiplier}-{data_id}", state
        else:
            return sig.squeeze(0), sig_clean.squeeze(0), label, f"{label_multiplier}-{data_id}"
