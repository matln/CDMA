    ...
    def __init__(self, data_source, batch_size, num_chunks):
        if batch_size < num_chunks:
            raise ValueError(
                ...
                'than num_chunks={}'.format(batch_size, num_chunks)
            )

        self.data_source = np.squeeze(data_source.label)
    ...
    def __iter__(self):
        ...
        while len(avai_spks) > 0:
            if len(avai_spks) >= self.num_spks:
                ...
                current_spks = random.sample(list(set(self.spks) - set(avai_spks)),
                                                self.num_spks - len(avai_spks))
