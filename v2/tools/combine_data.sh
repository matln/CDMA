...
if $has_segments; then
    for in_dir in $*; do
        if [ ! -f $in_dir/segments ]; then
            ...
            get_segments_for_data.sh $in_dir
        else
            cat $in_dir/segments
        fi
...
for file in utt2spk utt2lang utt2dur utt2num_frames reco2dur feats.scp text cmvn.scp vad.scp reco2file_and_channel wav.scp spk2gender utt2domain $extra_files; do
...
utt2spk_to_spk2utt.pl <$dest/utt2spk >$dest/spk2utt
[[ -f $dest/utt2domain ]] && utt2spk_to_spk2utt.pl <$dest/utt2domain >$dest/domain2utt

if [[ $dir_with_frame_shift ]]; then
    cp $dir_with_frame_shift/frame_shift $dest
fi

if [ -f $dest/utt2domain ]; then
	utt2spk_to_spk2utt.pl <$dest/utt2domain >$dest/domain2utt
fi

fix_data_dir.sh $dest || exit 1
