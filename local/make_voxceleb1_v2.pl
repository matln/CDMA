...
if (@ARGV != 3) {
  print STDERR "Usage: $0 <path-to-voxceleb1> <data> <path-to-data-dir>\n";
  print STDERR "e.g. $0 /export/voxceleb1 dev data/dev\n";
  exit(1);
}

($data_base, $data, $out_dir) = @ARGV;

if ("$data" ne "dev" && "$data" ne "test" && "$data" ne "test_nospk") {
  die "data parameter must be 'dev' or 'test'!";
}

if ("$data" eq "dev" || "$data" eq "test") {
  $dataset = $data;
}

if ("$data" eq "test_nospk") {
  $dataset = "test";
}
...
if ($data eq "test_nospk"){
  ...
  while (<TRIAL_IN>) {
    ...
    my ($spkr_id, $rec_id, $name) = split('/', $path1);
    $name =~ s/\.wav$//g;
    my $utt_id1 = "$rec_id-$name";
    $test_spkrs{$spkr_id} = ();

    # Create entry for right-hand side of trial
    my ($spkr_id, $rec_id, $name) = split('/', $path2);
    $name =~ s/\.wav$//g;
    my $utt_id2 = "$rec_id-$name";
    $test_spkrs{$spkr_id} = ();
