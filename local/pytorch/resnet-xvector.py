...
class ResNetXvector(TopVirtualNnet):
    ...
    def init(self, inputs_dim, num_targets, t_num_targets, aug_dropout=0., tail_dropout=0., training=True,
             ...
             t_margin_loss=False, t_margin_loss_params={}, use_step=False, step_params={},
             t_use_step=False, t_step_params={}, transfer_from="softmax_loss", batch_size=128,
             num_chunks=8, mode="pretrain"):
        ...
        t_margin_loss_params = utils.assign_params_dict(default_margin_loss_params, t_margin_loss_params)
        step_params = utils.assign_params_dict(default_step_params, step_params)
        t_step_params = utils.assign_params_dict(default_step_params, t_step_params)

        # Var.
        self.extracted_embedding = extracted_embedding  # only near here.
        self.use_step = use_step
        self.step_params = step_params
        self.t_use_step = t_use_step
        self.t_step_params = t_step_params
        ...
        if training:
            ...
            if mode not in ["pretrain", "finetuning"]:
                self.mmd_loss = MaximumMeanDiscrepancy(batch_size, num_chunks, mode)

            if mode in ["SDA", "finetuning"]:
                if t_margin_loss:
                    self.t_loss = MarginSoftmaxLoss(resnet_params["planes"][3], t_num_targets, **t_margin_loss_params)
                else:
                    self.t_loss = SoftmaxLoss(resnet_params["planes"][3], t_num_targets)
    ...
    def get_t_loss(self, inputs, targets):
        return self.t_loss(inputs, targets)
    ...
    def get_t_accuracy(self, targets):
        ...
        """
        return self.t_loss.get_accuracy(targets)
    ...
    def step(self, epoch, this_iter, epoch_batchs):
        ...
        if self.t_use_step:
            if self.t_step_params["m"]:
                current_postion = epoch * epoch_batchs + this_iter
                lambda_factor = max(self.t_step_params["lambda_0"],
                                    self.t_step_params["lambda_b"]*(1+self.t_step_params["gamma"]*current_postion)**(-self.t_step_params["alpha"]))
                self.t_loss.step(lambda_factor)

            if self.t_step_params["T"] is not None and (self.t_step_params["t"] or self.t_step_params["p"]):
                T_cur, T_i = self.get_warmR_T(*self.t_step_params["T"], epoch)
                T_cur = T_cur*epoch_batchs + this_iter
                T_i = T_i * epoch_batchs

            if self.t_step_params["t"]:
                self.t_loss.t = self.compute_decay_value(*self.t_step_params["t_tuple"], T_cur, T_i)

            if self.t_step_params["s"]:
                self.t_loss.s = self.t_step_params["s_tuple"][self.t_step_params["s_list"][epoch]]
