    ...
    def __init__(
        ...
        if not head_conv and self.in_planes != head_inplanes:
            raise ValueError(
                ...
                "inplanes without head conv({} vs. {}).".format(
        ...
        if self.head_conv:
            ...
            self.conv1 = self.Conv(
                head_inplanes, self.inplanes, **head_conv_params, bias=False
            )
        ...
        for m in self.modules():
            if isinstance(m, self.Conv):
                ...
                isinstance(m, (nn.BatchNorm1d, nn.BatchNorm2d, nn.GroupNorm))
                and norm_layer_affine
            ):
