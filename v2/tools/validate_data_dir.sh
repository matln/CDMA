...
! cmp -s <(cat $data/utt2spk | awk '{print $1, $2;}') \
     <(spk2utt_to_utt2spk.pl $data/spk2utt)  && \
...
if [ -f $data/text ]; then
  validate_text.pl $data/text || exit 1;
...
for f in vad.scp utt2lang utt2uniq utt2domain; do
  if [ -f $data/$f ]; then
...
if [ -f $data/domain2utt ]; then
	ns=$(wc -l < $data/domain2utt)
	if [ "$ns" == 1 ]; then
		echo "$0: WARNING: you have only one key for domain2utt. This probably a bad idea."
	fi

	check_sorted_and_uniq $data/domain2utt

	! cmp -s <(cat $data/utt2domain | awk '{print $1, $2;}') \
		 <(spk2utt_to_utt2spk.pl $data/domain2utt | sort -k1,1 -u)  && \
	   echo "$0: domain2utt has redundant utts" && exit 1
fi
