...
#     steps_done=$(cat $tmpfile)
#     while [[ $steps_done == "" ]]; do
#         steps_done=$(cat "$tmpfile")
#     done
# 
#     steps=$(echo "$steps_done - $last_step" | bc)
#     # 1% steps
#     if [[ $steps -ge $(echo "$total_steps / 100" | bc) ]]; then
#         end_time=`date +%s.%N`
#         used_time=$(echo "$end_time - $start_time" | bc)
#         remain_time=$(echo "$used_time * ($total_steps - $steps_done) / $steps" | bc)
...
#         printf "progress: [%-50s] %s%% %s\r" "${mark}" "${percent}" "${remain_time}"
#     fi
# 
# done
