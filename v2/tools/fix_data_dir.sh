...
if [ $# != 1 ]; then
  ...
  echo "e.g.: fix_data_dir.sh data/train"
...
for x in utt2spk spk2utt feats.scp text segments wav.scp cmvn.scp vad.scp \
	...
	utt2num_frames utt2domain domain2utt $utt_extra_files $spk_extra_files; do
	if [ -f $data/$x ]; then
...
function filter_utts {
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2dur utt2num_frames utt2domain $maybe_wav $maybe_reco2dur $utt_extra_files; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
...
if [[ -f "$data/utt2domain" ]]; then
	[ ! -f "$data/domain2utt" ] && echo "$0: line $LINENO: utt2domain and domain2utt must exist together" && exit 1
	! cmp -s <(cat $data/utt2domain | awk '{print $1, $2;}') \
		 <(spk2utt_to_utt2spk.pl $data/domain2utt | sort -k1,1 -u)  && \
	   utt2spk_to_spk2utt.pl $data/utt2domain >$data/domain2utt && \
	   sort -k1,1 -u <$data/domain2utt >$data/domain2utt.tmp && mv $data/domain2utt.tmp $data/domain2utt
fi
