...
class Conv1dReluBn(nn.Module):
    ...
    """
    def __init__(
        self, inputs_dim, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, bias=True
    ):
        super().__init__()
        self.conv = nn.Conv1d(
            inputs_dim, out_channels, kernel_size, stride, padding, dilation, bias=bias
        )
...
class ECAPA_TDNN(TopVirtualNnet):
    def init(
        ...
        if pooling == "attentive":
            ...
            self.fc1 = (
                ReluBatchNormTdnnLayer(cat_channels * 2, embd_dim, **fc1_params) if fc1 else None
            )
            ...
            self.fc1 = (
                ReluBatchNormTdnnLayer(
                    cat_channels * 2 * pooling_params["num_head"], embd_dim, **fc1_params
                )
                if fc1
                else None
            )
